...
void setup()
{
  ...
  DBG(F("OK, now make sure an rtpMIDI session is Enabled"));
  DBG(F("Add device named Arduino with Host"), WiFi.localIP(), "Port", AppleMIDI.getPort());
...
void loop()
{
  ...
  // (do not call delay(1000) as it will stall the pipeline)
  if ((isConnected > 0) && (millis() - t0) > 100)
  {
    static bool first = true;

    if (first) {
      DBG(F("first MIDI.sendNoteOn()"));
      first = false;
    }
...
void OnAppleMidiException(const APPLEMIDI_NAMESPACE::ssrc_t& ssrc,
  ...
  switch (e) {
    ...
    case APPLEMIDI_NAMESPACE::Exception::NoResponseFromConnectionRequestException:
      DBG(F("***:yyy did't respond to the connection request. \
            Check the address and port, and any firewall or router settings. (time)"));
      break;
